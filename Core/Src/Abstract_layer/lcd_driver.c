...
void HD44780_Init(uint8_t rows) {
	...
	ExpanderWrite(dpBacklight); // se trimite 0 la cum e acum
	HAL_Delay(1000);
...
static void DelayInit(void) {
	...
	CoreDebug->DEMCR |= CoreDebug_DEMCR_TRCENA_Msk;  //Data Watch point trigger enable bit DWT)

	DWT->CTRL &= ~DWT_CTRL_CYCCNTENA_Msk; //~0x00000001;
	DWT->CTRL |= DWT_CTRL_CYCCNTENA_Msk; //0x00000001;  enable cycle counter

	DWT->CYCCNT = 0; //holds the cycle counter

	/* 3 NO OPERATION instructions */
	__ASM volatile ("NOP");
	__ASM volatile ("NOP");
	__ASM volatile ("NOP"); // DWT->CYCNT = 3; 3 cycluri in care nu se intampla nimic.
}
